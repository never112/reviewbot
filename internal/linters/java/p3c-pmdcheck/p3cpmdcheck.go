...
func pmdCheckHandler(ctx context.Context, a linters.Agent) error {
	...
	if checkerr != nil {
		plog.Errorf("p3c-pmd rule check failed: %v", checkerr)
		return checkerr
	}
	a = argsApply(plog, a)
...
func argsApply(log *xlog.Logger, a linters.Agent) linters.Agent {
	...
	//  java -cp java net.sourceforge.pmd.PMD  -d D:\CodeProject\demo2  -R D:\CodeProject\p3c\p3c-pmd\src\main\resources\rulesets\java\ali-comment.xml,D:\CodeProject\p3c\p3c-pmd\src\main\resources\rulesets\java\ali-oop.xml
	//java -cp p3c-pmd-2.1.1-jar-with-dependencies.jar net.sourceforge.pmd.PMD -d /Users/huey/dev/java/ideaWorkSpace/rongc/zx-bme/staff-zx-service/staff-zx-pfmc/src/main/java/com/sunacwy/staff/zx/pfmc -R rulesets/java/ali-comment.xml
	log.Info("p3c-pmdcheck comamnd:" + strings.Join(config.Command, " "))
	if linters.IsEmpty(config.Args...) {
		args := append([]string{}, "-cp")
		args = append(args, "p3c-pmd-2.1.1-jar-with-dependencies.jar", "net.sourceforge.pmd.PMD")
		args = append(args, "-R", "net.sourceforge.pmd.PMD", "rulesets/java/ali-comment.xml")
		config.Args = args
	}
